...
class TrafficFeatureExtractor:
    ...
    def extract_normal_traffic(n_samples=10000):
        ...
        for _ in range(n_samples):
            packet_size = int(np.random.gamma(2, 500))
            protocol = np.random.choice([6, 17], p=[0.85, 0.15])
            ...
            day = int(np.random.normal(3, 2)) % 7
            ...
            entropy = np.random.beta(2, 8)
            
            interval = np.random.exponential(50)
