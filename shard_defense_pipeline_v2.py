#!/usr/bin/env python3
"""
SHARD Defense Pipeline v2.1 — с ML-классификатором (TfidfVectorizer + XGBoost) и keyword matching fallback
"""
...
class DefenseModelLoader:
    def __init__(self, model_path='./models/defense_classifier_v3.pkl'):
        ...
        self._keywords = {
            'SQL Injection': ['sql', 'injection', 'sqli', 'union', 'select', 'mysql', 'postgresql', 'blind', 'error-based', 'time-based', 'out-of-band', 'second-order'],
            'Brute Force': ['brute', 'force', 'ssh', 'ftp', 'login', 'password', 'hydra', 'dictionary', 'credential', 'stuffing', 'rdp', 'telnet'],
            'DDoS': ['ddos', 'syn', 'flood', 'udp', 'icmp', 'amplification', 'slowloris', 'http flood', 'memcached', 'ntp', 'dns amplification'],
            'Port Scan': ['port', 'scan', 'nmap', 'scanning', 'probe', 'masscan', 'syn scan', 'connect scan', 'udp scan', 'aggressive scan', 'service version'],
            'C2 Beacon': ['c2', 'beacon', 'command', 'cobalt', 'meterpreter', 'empire', 'sliver', 'reverse shell', 'periodic', 'websocket'],
            'DNS Tunnel': ['dns', 'tunnel', 'exfil', 'iodine', 'dnscat', 'dns over https', 'txt record', 'high-entropy dns', 'dns beaconing'],
            'XSS': ['xss', 'script', 'javascript', 'onerror', 'alert', 'cross-site', 'dom', 'stored', 'reflected', 'polyglot', 'mutation'],
            'Lateral Movement': ['lateral', 'movement', 'smb', 'rdp', 'psexec', 'wmi', 'pass-the-hash', 'winrm', 'scheduled task', 'service creation'],
            'Data Exfiltration': ['exfil', 'data', 'upload', 'transfer', 'leak', 'dump', 'archive', 'cloud storage', 'ftp exfiltration', 'encrypted channel'],
            'Botnet': ['botnet', 'bot', 'zombie', 'mirai', 'gafgyt', 'mozi', 'emotet', 'trickbot', 'qakbot', 'iot'],
            'Ransomware': ['ransom', 'encrypt', 'locker', 'wannacry', 'ryuk', 'lockbit', 'conti', 'revil', 'blackcat', 'ransom note'],
            'Phishing': ['phish', 'fake', 'credential', 'spoof', 'social', 'harvesting', 'spear', 'clone', 'whaling', 'business email', 'malicious redirect'],
            'Zero-Day': ['zero', 'unknown', 'new', 'suspicious', 'novel', 'zero-day', 'unclassified', 'emerging', 'previously unseen', 'anomalous'],
        }
        self._load()
        ...
        try:
            if self.model_path.exists():
                ...
                if self.vectorizer is not None and self.classifier is not None and self.label_encoder is not None:
                    ...
                    logger.info(f"✅ Defense ML модель загружена: {model_type} v{model_version}, {feature_count} признаков, {len(self.attack_types)} классов")
                else:
...
class ModelCodeGen:
    def __init__(self, model: DefenseModelLoader):
        ...
        self.templates = {
            ...
            'Ransomware': 'iptables -A INPUT -s {ip} -j DROP\niptables -A OUTPUT -d {ip} -j DROP\niptables -A OUTPUT -p tcp --dport 445 -j DROP',
            'XSS': 'iptables -A INPUT -s {ip} -p tcp --dport {port} -j DROP\nSecRule REQUEST_URI "@rx <script" "id:2001,phase:2,deny,status:403"',
            'Lateral Movement': 'iptables -A INPUT -s {ip} -p tcp --dport 445 -j DROP\niptables -A INPUT -s {ip} -p tcp --dport 139 -j DROP\niptables -A FORWARD -s {ip} -j DROP',
            'Data Exfiltration': 'iptables -A OUTPUT -d {ip} -p tcp --dport {port} -j DROP\niptables -A OUTPUT -d {ip} -j DROP',
            'Phishing': 'iptables -A INPUT -s {ip} -p tcp --dport 80 -j DROP\niptables -A INPUT -s {ip} -p tcp --dport 443 -j DROP',
        }
    ...
    def generate(self, atype: str, alert: Dict) -> str:
        ...
        template = self.templates.get(atype, self.templates['Zero-Day'])
...
class DefensePipeline:
    def __init__(self):
        ...
        self.ethical = None
        self.stats = {'alerts': 0, 'defense_generated': 0}
    ...
    def process_alert(self, alert: Dict) -> Dict:
        with self._lock:
            ...
            return {'attack_type': atype, 'confidence': conf, 'code': code}
