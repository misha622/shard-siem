...
from app.email_service import email_service, EmailMessage, EmailEvent, EmailPriority, EmailTemplate
...
async def get_settings(current_user: dict = Depends(get_current_user)):
    settings = await db.get_email_settings(current_user["id"])
...
async def update_settings(data: EmailSettingsUpdate, current_user: dict = Depends(get_current_user)):
    await db.update_email_settings(current_user["id"], data.settings)
    settings = await db.get_email_settings(current_user["id"])
...
async def send_test(data: TestEmailRequest, current_user: dict = Depends(get_current_user)):
    ...
    msg = EmailMessage(
        ...
        body_html=EmailTemplate.base_template(f"<p style='color:#fff'>{data.message}</p>","Test"),
        event_type=EmailEvent.CUSTOM
    )
